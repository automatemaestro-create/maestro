...
| 28 | [Frontière d'exécution d'un run : note de décision](./docs/28-decision-frontiere-execution-run.md) | **L'hôte de run détaché**, pour qu'un run survive à l'arrêt de l'API : les deux pannes du 14 août distinguées, la file écartée sur un fait (elle déplace les tâches, pas le run), Temporal chiffré et mis derrière une porte nommée, et le chantier en six lots |
