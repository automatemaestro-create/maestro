...
| 11 | [Démo de bout en bout du POC](./docs/11-demo-poc.md) | Archive de la Phase 0 : le parcours objectif → tâches → agents → fichiers, et la validation du critère de sortie. La commande `maestro-demo` a quitté le produit (#1168) ; le banc des scénarios de référence (`python -m maestro.scenarios`, #1148) lui succède |
...
| 28 | [Frontière d'exécution d'un run : note de décision](./docs/28-decision-frontiere-execution-run.md) | **L'hôte de run détaché**, pour qu'un run survive à l'arrêt de l'API : les deux pannes du 14 août distinguées, la file écartée sur un fait (elle déplace les tâches, pas le run), Temporal chiffré et mis derrière une porte nommée, et le chantier en six lots — **livré** le 2026-08-24, avec ce qu'il a coûté et les trois corrections en route (§10). La **cinquième porte** (§11) y ajoute l'arrêt volontaire : un run survit à l'accident, pas à l'extinction — et **fermer la fenêtre du navigateur en est une** depuis le 2026-08-28 (§11.2), l'accident toléré se réduisant au redémarrage |
| 29 | [Le run, objet de premier plan : note de décision](./docs/29-decision-run-objet-de-premier-plan.md) | **Trois arbitrages** rendus sur la revue d'usage du 2026-08-24 : le **Kanban passe dans le run** (le run devient une portée d'écran à côté du projet), le **chat devient la seule porte d'entrée** (le brief y déménage, D5 tient) et l'**arrêt volontaire solde les runs**. Ce que chacun coûte en travail déjà livré, mesuré sur le dépôt — et les sept trous de la revue, avec leur ticket |
| 31 | [La surface d'écriture des agents pendant un run : note de décision](./docs/31-decision-surface-ecriture-agents.md) | **Une règle appliquée cinq fois** : un agent écrit ce qu'il *observe*, jamais ce que le plan *décide*. Deux verbes ouverts (déclarer un blocage, écrire à un pair), trois refusés parce qu'ils amendent le plan validé — que seul un point d'approbation humain amende (D5). Le **canal existait déjà** (serveur MCP in-process, #582) et l'**identité d'instance est écartée** : `tache_id` la porte déjà |
...
| 38 | [L'outillage universel d'un projet : le format, sa place, et la frontière](./docs/38-decision-outillage-universel-du-projet.md) | **La question laissée ouverte par docs/37 §4.5, tranchée sur vérification.** Aucun chemin de skills n'est lu par les quatre clients : `.agents/skills/` en réunit **trois sur quatre** et ne porte le nom d'aucun éditeur, donc c'est lui — le quatrième est joint par la **désignation** dans `AGENTS.md`, jamais par une copie ni un lien symbolique. `AGENTS.md` seul n'atteint pas non plus les quatre (Gemini ne le lit pas par défaut, Claude Code seulement sans `CLAUDE.md`) : **deux ponts d'une ligne**. **Aucune commande générée** — trois clients, trois formats, aucun commun, et un skill s'appelle déjà par son nom. Un **manifeste** dit ce qui a été généré et d'où, et régénère sans écraser (empreinte à la génération, quatre cas). Et la frontière, mesurée : `strict_mcp_config` ne ferme que les serveurs MCP — `setting_sources` n'est passé **nulle part**, donc un `CLAUDE.md` de projet entre aujourd'hui dans le contexte de nos agents. C'est ce jalon qui rend le trou atteignable ; #1032 le referme |
