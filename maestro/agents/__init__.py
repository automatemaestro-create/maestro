...
le Développeur (`DEVELOPER_PROFILE`, ticket #4) et la Base de données
(`DATABASE_PROFILE`, ticket #5). Ajouter un rôle outillé = déclarer un profil et
l'inscrire dans `TOOLED_PROFILES`.
"""
...
TOOLED_PROFILES: tuple[RoleProfile, ...] = (DEVELOPER_PROFILE, DATABASE_PROFILE)
...
def default_runtimes(provider: ModelProvider) -> dict[str, AgentRuntime]:
    ...
    l'un de ces noms (`developpeur`, `bdd`) s'exécute via son runtime outillé plutôt
    que par un appel texte.
    """
