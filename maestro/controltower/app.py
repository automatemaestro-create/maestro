...
- `GET  /api/catalogue` — le catalogue d'agents (#72, EF-03) : les agents par
  ...
  serveurs MCP effectifs (#104 — `mcp_serveurs`/`mcp_erreur`), ce que leur
  **seule déclaration héritée** porte encore (#263 — `mcp_herites`, ce que la
  migration ferait passer au pool) et leur politique de permissions effective
  (#110, lecture seule — `permissions`/`permissions_erreur`) ;
