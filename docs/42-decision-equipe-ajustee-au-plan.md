...
- ⚠ **« Continuer avec l'équipe qu'il a » veut dire que ses tâches trouvent quelqu'un** (#1260). Décliner, ne pas répondre ou n'avoir personne à qui proposer laissait partir les tâches du métier absent « à assigner » : le classifieur, interrogé sur *qui est compétent*, s'abstenait honnêtement. Tout l'aval du plan se bloquait derrière, et le bouclage du 2026-09-24 a vu un run finir 0/3 sans rien produire. Ces tâches vont désormais au **rôle le plus proche**, désigné par le classifieur à une question qui n'admet pas l'abstention (`maestro/router/router.py`, `au_plus_proche`). Il en va de même d'un **second métier absent**, puisqu'on n'en propose qu'un par run : personne ne l'a jamais proposé, et le run `2e7f7278a991` (S6) finissait 2/3 sur sa tâche de QA alors que le Designer avait été accepté. Le journal le dit à côté du recrutement. Le repli « à assigner » de #42 reste entier hors de ces cas.
- ⚠ **Accepter donne au rôle recruté les tâches qui demandaient son métier** (#1260). Le fil le promet en recrutant, et le routage ne le tenait pas toujours. Une tâche `ui` + `frontend` faisait un ex æquo entre le développeur et le designer recruté, et le classifieur le tranchait pour le premier (run `2f7aae8437f4`, S6). Les compétences recrutées pour le plan restreignent désormais les candidats d'une tâche qui les demande à ceux qui les couvrent (`recrutees`).
- **La proposition passe par le bus, quel que soit l'hôte du run** (#1260). Elle était écrite dans le fil par l'arbitre, ce qui ne marchait que dans le process de l'API. La vraie stack, dont les runs vivent dans l'hôte détaché, ne la montrait donc jamais. Le détail est en [docs/28 §3](./28-decision-frontiere-execution-run.md), et le scénario de référence S6 la rejoue sur le réel ([docs/40 §5](./40-decision-rythme-et-scenarios-de-reference.md)).
