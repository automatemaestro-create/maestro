...
| Livrables d'un run | Capturés en mémoire (`ProducedFile`, texte ≤ 1 Mo). Seul `maestro-demo` les **réécrit** quelque part (`sortie-demo/<run>/livrables/` — commande et dossier retirés depuis par #1168) : un run lancé **depuis la Control Tower n'écrit aucun fichier sur le disque** — le contenu ne vit que dans la projection et le journal | L'utilisateur reçoit **un compte rendu**, pas un projet |
