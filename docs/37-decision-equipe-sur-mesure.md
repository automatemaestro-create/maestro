...
3. **Les autorisations sont proposées par l'analyse, puis validées par l'utilisateur** (#1039, #1040). Chaque permission `auto` est nommée avec sa raison. Une permission `auto` reste donc une décision humaine prise à l'avance ; seule sa **rédaction** devient automatique.
   > ⚠ **Ce que #1226 a corrigé le 2026-09-23.** L'analyse proposait `Bash: humain` dès qu'aucune commande du rôle n'avait été lue dans le projet, c'est-à-dire **toujours** sur un projet neuf : un `dev` recruté pour écrire du code devait faire approuver le fait de le lancer (run `96d0c3482649` : 14 demandes, 14 approbations — `mkdir`, `python`, `pytest`, et le ménage de ses propres caches). Un rôle validé reçoit désormais une autorisation d'**exécuter dans son projet** — cran `auto`, **portée `projet`** ([docs/32 §8](./32-decision-cran-orchestrateur.md), porte 2) —, et la raison servie à la validation dit les deux moitiés : ce qu'il fait seul, et ce qui vous revient quand même (sortir du dossier, effacer ce que vous y aviez posé). La phrase de cet arbitrage tient telle quelle — c'est toujours une décision humaine prise d'avance, et ce qui a changé est **ce qu'on propose de décider**, pas qui décide.
...
| Le **routage sur l'équipe** : une règle pour ses deux lecteurs, le catalogue remplacé pour l'appel, l'équipe **vide** qui n'est pas une omission, et ce que personne ne couvre nommé en **poste**. ⚠ Le signal ne redit plus que « le recrutement se fait hors du run » : [docs/42](./42-decision-equipe-ajustee-au-plan.md) l'a renversé (#1227), et au **routage** il est de toute façon trop tard — la tâche part « à assigner » | [`tests/test_equipe_routage.py`](../tests/test_equipe_routage.py) |
| L'**équipe confrontée au plan** (#1227, [docs/42](./42-decision-equipe-ajustee-au-plan.md)) : la décomposition nomme le métier qu'une tâche demande même hors de l'équipe, le manque du plan est calculé et consigné, le rôle est proposé dans le fil avec ses tâches, et accepter / décliner / ne pas répondre laissent toutes le run aboutir | [`tests/test_equipe_au_plan.py`](../tests/test_equipe_au_plan.py) |
