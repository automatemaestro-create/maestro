...
- **`_cadre_outille.md` — le cadre d'exécution outillée** : répertoire de travail (le répertoire courant, qui depuis #839 peut être la racine d'un projet — il n'est pas promis isolé, cf. [docs/24 §2.4](./24-projets-locaux-et-poste-de-travail.md)), livrable **matérialisé en fichiers**, rien de destructif au-dehors, aucun processus qui survive à la tâche. Il dit aussi **avec quoi lire** : *lire n'est pas exécuter* — un fichier s'ouvre avec `Read` (et `Glob`/`Grep` pour chercher), jamais par une commande shell, qui peut attendre l'accord d'une personne que lire ne demande pas ; et un **appel d'outil refusé est une décision**, qui ne se rejoue pas à l'identique (#1102, [docs/38 §5.5](./38-decision-outillage-universel-du-projet.md)). Réservé aux runtimes ; l'exécution texte du catalogue n'a pas d'outils et ne le charge pas.
