...
**Un message trop gros n'est pas un aléa** (#1277). Le SDK lit le flux du CLI message par message sous un plafond (`max_buffer_size`), de 1 Mio par défaut ; une capture d'écran relue par `Read` le franchit, parce que le CLI écrit l'image redimensionnée, en base64, **deux fois** dans la même ligne (mesuré : un PNG de 331 886 octets fait une ligne de 1 186 283 octets). Le fournisseur pose donc ce plafond sur **toutes** ses sessions (`providers.claude.PLAFOND_FLUX_OCTETS`, 64 Mio : deux fois les 32 Mo qu'une requête à l'API peut porter), et mue un dépassement en `PlafondFluxDepasse`, que `est_transitoire` exclut : le fichier trop gros est encore sur le disque à la tentative suivante, et le relancer ne ferait que rejouer l'échec — ce qu'a fait le run `3fe501fc0878`, deux fois, avant de le raconter comme « transitoire ».
...
2. **Human-in-the-loop** — un **appel d'outil** classé `ask` par la politique de permissions de l'agent (`core/permissions/<agent>.json`) est suspendu au vol et attend une décision explicite dans l'UI. Le déclencheur est l'**acte** que l'agent s'apprête à commettre, pas le texte de la tâche : développer une fonction de suppression n'est pas exécuter une suppression (chantier #573, [docs/04 §1.4bis](./04-specifications-agents.md)). Qui tranche est posé dans la politique — `auto` ou `humain`, ce dernier par défaut ; sans personne pour trancher, l'acte est **refusé**. ⚠ Un troisième cran, `orchestrateur`, a été **retiré** ([docs/32](./32-decision-cran-orchestrateur.md), décision #647, retrait #715) : il n'a jamais eu de canal, si bien que l'y poser rendait un refus — *la politique promettait une décision et rendait un refus*. Une politique qui l'écrit échoue désormais au chargement, et les actes qui lui revenaient remontent au **défaut**, `humain` : `auto` n'hérite de rien.
