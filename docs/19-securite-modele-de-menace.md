...
|---|---|---|
| **Processus principal** — Node complet, droits de l'utilisateur (`apps/desktop/main.js`) | Il n'exécute **qu'un** programme, `scripts/controltower/start.sh`, jamais une commande venue de la page ; ce qu'il lui transmet est **fixe** — `--no-browser` pour démarrer, `--stop` pour arrêter —, et **aucune** option de son `argv` — celui d'Electron — ne passe (la liste blanche `--demo` est partie avec le mode démo, #1168) ; l'interpréteur est celui que le lanceur lui passe (`MAESTRO_BASH`), pas un `bash` nu qui sous Windows tomberait sur WSL ; une seconde instance **de la même stack** sort immédiatement (`requestSingleInstanceLock`, tenu sur un profil propre à chaque couple de ports — la clé dont `start.sh` indexe sa stack, #1275 : deux worktrees ouvrent deux fenêtres, la même copie n'en ouvre qu'une) | Ce processus **a tous les droits de l'utilisateur**, et rien ne l'en prive : ni conteneur, ni jeton, ni politique d'outils. C'est lui la frontière — il ne peut pas être derrière elle. Le lanceur est joué depuis la racine du dépôt (`cwd`) et hérite de l'environnement de la coque tel quel |
