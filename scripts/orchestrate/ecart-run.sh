...
voulu	bash	while read f; do echo $f; done	#829 · #528 · mesuré 1× (#267)	IL SUIT « for » — question posée, pas supposée — et ajoute une raison PROPRE, qui pèse plus lourd : un « for » itère une liste ÉCRITE, qu'on lit dans la commande même ; un « while » itère sur une CONDITION, donc rien ne le borne, et « while true » est la boucle sans fin que ce fichier existe pour empêcher (#169). Son jumeau « until [ -s … ]; do sleep 3; done » est refus mérité depuis #179. Forme couverte : celle de « for », à l'identique.
...
voulu	bash	mv a b	#829 · mesuré 1× (#263)	Même famille que « rm » et « git clean » : aucune règle de préfixe ne borne sa CIBLE, donc « Bash(mv:*) » sortirait du worktree — et il ÉCRASE sa destination sans rien dire, un pouvoir que « rm » n'a même pas. Ce qui l'écarte de son voisin INSTRUIT est que « git mv » est borné PAR GIT (source suivie, destination dans le dépôt), une borne qui ne tient à aucune règle. Formes couvertes : « git mv » pour un fichier suivi (#790), « Write » au nouveau chemin pour un fichier que la session vient d'écrire. Mesure : la MÊME session a vu tomber « git mv », pourtant autorisé — les deux appels portaient un « cd <chemin absolu> », c'est-à-dire l'échappée de #307, que ce verdict ne corrige pas.
