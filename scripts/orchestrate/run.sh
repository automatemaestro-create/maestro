...
# patiente <iid> <secondes> : attend, en tranches, pour que le fichier STOP reste entendu pendant une
# attente qui peut durer des heures. Renvoie 1 si l'arrêt a été demandé.
patiente() {
  local iid="$1" reste="$2" tranche fin
  fin="$(date -d "+$reste seconds" '+%H:%M' 2>/dev/null || echo '?')"
  # `dit` et non `printf` : la ligne passe par la file du pilote (#290), seul écrivain de l'écran.
  ...
  # pas son rouet. La colonne « durée », elle, reste celle du TICKET — c'est elle qu'on suit d'un
  # bout à l'autre —, et le temps d'attente est dit en clair dans le détail.
  while [ "$reste" -gt 0 ]; do
    [ -f "$STOP" ] && return 1
    vue_publie "$iid" '=' \
...
renonce_au_run() {
  ...
  for f in "$RUN_DIR"/* "$RUN_DIR"/.[!.]*; do
    ...
    case "${f##*/}" in plan.tsv | reprise-de | pid | .console) ;; *) return 1 ;; esac
  done
...
if [ "$CONCURRENCE" -gt 1 ]; then
  printf '%s%s tickets en vol%s — deux ne partent ensemble que si le plan les dit indépendants.\n' \
    "$C_B" "$CONCURRENCE" "$C_0"
  # La limite qui reste de ce lot, dite une fois plutôt que découverte à l'écran (#291). Celle de la
  # vue est levée : le bloc rend les N tickets en vol depuis #290.
