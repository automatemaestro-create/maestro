...
- `/ticket-create <type> <titre>` — crée un ticket bien formé (corps de template + labels `type::`/`agent::`/`prio::`), statut `À faire` par défaut. **Évalue la taille** : au-delà d'~1 session, crée un **parent de suivi + sous-tickets liés** (découpage, docs/10 §5.1) ; sinon ticket unique, avec checklist interne s'il est multi-facettes. Ne crée pas de branche (c'est le rôle de `/ticket-start`).
